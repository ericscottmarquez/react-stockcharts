...
function updateChart(
    ...
    if (!lastItemWasVisible || end >= xAccessor(lastItem)) {
        ...
        const dx =
            initialXScale(xAccessor(lastItem)) - initialXScale.range()[1];
...
class ChartCanvas extends Component {
    ...
    amIOnTop(id) {
        ...
        return (
            dragableComponents.length > 0 && last(dragableComponents).id === id
        );
    }
    ...
    pinchZoomHelper(initialPinch, finalPinch) {
        ...
        const { topLeft: iTL, bottomRight: iBR } =
    ...
    componentWillReceiveProps(nextProps) {
        ...
        if (
            ...
            const lastItemWasVisible =
                xAccessor(prevLastItem) <= end &&
                xAccessor(prevLastItem) >= start;
        ...
        return (
            ...
            >
                ...
                <svg
                    ...
                    width={width}
                    height={height}
                    style={{ position: "relative", zIndex: zIndex + 5 }}
                >
                    {cursor}
                    <defs>
